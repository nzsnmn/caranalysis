...
public class HttpsProxy {
    ...
    public byte[] doPostFroByte(String url, String params,int connectTimeout, int readTimeout) throws Exception {
        String ctype = "application/json;charset=" + DEFAULT_CHARSET;
        byte[] content = {};
        if(params != null){
            content = params.getBytes(DEFAULT_CHARSET);
        }
        return doPostForByte(url, ctype, content, connectTimeout, readTimeout);
    }
    ...
    public byte[] doPostForByte(String url, String ctype, byte[] content,
                         int connectTimeout,int readTimeout) throws Exception {
        HttpsURLConnection conn = null;
        OutputStream out = null;
        byte[] rsp = new byte[0];
    ...
    private byte[] getStreamAsByte(InputStream stream) throws IOException {
        try {
